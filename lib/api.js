...
export default function createApi(accessToken) {
  ...
  return {
    ...
    search(options) {
      return axios
        .get(`/search?` + new URLSearchParams(options).toString())
        .then(({ data }) => data);
    },

    searchBusiness(searchTerm) {
      return axios
        .get(`/searchBusiness?q=${searchTerm}`)
        .then(({ data }) => data);
    },
    ...
    fetchBusinessOrders(id) {
      return axios.get(`/businesses/${id}/statements`).then(({ data }) => data);
    },
    ...
    chargeCredit(monetaryAmount) {
      return axios
        .post("/insertCredit", {
          amount: monetaryAmount.saldo.amount,
          currency: monetaryAmount.saldo.currency
        })
        .then(({ data }) => data);
    },
